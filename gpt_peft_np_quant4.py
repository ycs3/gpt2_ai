...
def quantize_affine_int4(weight, group_size):
    if weight.ndim < 2 or weight.shape[-1] % group_size != 0:
        raise ValueError("Only tensors with a trailing dimension divisible by group_size can be quantized")

    flat_groups = weight.reshape(-1, group_size)
    mins = flat_groups.min(axis=1, keepdims=True)
    maxs = flat_groups.max(axis=1, keepdims=True)
    scales = (maxs - mins) / np.float32(15.0)
    zero_scale = scales.squeeze(-1) == 0
    safe_scales = scales.copy()
    safe_scales[zero_scale, 0] = 1.0
    quantized = np.round((flat_groups - mins) / safe_scales).clip(0, 15).astype(np.uint8)
    if np.any(zero_scale):
        quantized[zero_scale] = 0

    return {
        "packed": pack_int4(quantized),
        "scales": safe_scales.astype(np.float32).reshape(weight.shape[:-1] + (weight.shape[-1] // group_size,)),
        "biases": mins.astype(np.float32).reshape(weight.shape[:-1] + (weight.shape[-1] // group_size,)),
        "shape": weight.shape,
        "group_size": group_size,
        "num_values": int(quantized.size),
    }


def dequantize_affine_int4(entry):
    quantized = unpack_int4(entry["packed"], entry["num_values"]).reshape(-1, entry["group_size"]).astype(np.float32)
    scales = entry["scales"].reshape(-1, 1)
    biases = entry["biases"].reshape(-1, 1)
    return (quantized * scales + biases).reshape(entry["shape"]).astype(np.float32)


def should_quantize(weight, group_size):
    return weight.ndim >= 2 and weight.shape[-1] % group_size == 0


def get_weight(params, quantized_params, name):
    if name in quantized_params:
        return Tensor(dequantize_affine_int4(quantized_params[name]), requires_grad=False)
    return params[name]


def lora_linear(x, weight_name, bias, lora_a, lora_b, alpha, params, quantized_params):
    base = x @ get_weight(params, quantized_params, weight_name) + bias
...
def causal_self_attention(x, attn_mask, block, n_head, params, quantized_params):
    batch_size, seq_len, n_embd = x.data.shape
    head_dim = n_embd // n_head

    # GPT-2 attention input projection: hidden -> concatenated QKV.
    qkv = lora_linear(
        x,
        block["attn.c_attn.weight_name"],
    ...
    # GPT-2 attention output projection: attended states -> hidden size.
    return lora_linear(
        out,
        block["attn.c_proj.weight_name"],
...
def mlp(x, block, params, quantized_params):
    # GPT-2 feed-forward expansion: hidden -> 4x hidden.
    hidden = gelu(
        lora_linear(
            x,
            block["mlp.c_fc.weight_name"],
            block["mlp.c_fc.bias"],
    ...
    # GPT-2 feed-forward contraction: 4x hidden -> hidden.
    return lora_linear(
        hidden,
        block["mlp.c_proj.weight_name"],
...
def transformer_block(x, attn_mask, block, n_head, params, quantized_params):
    x = x + causal_self_attention(layer_norm(x, block["ln_1.weight"], block["ln_1.bias"]), attn_mask, block, n_head, params, quantized_params)
    x = x + mlp(layer_norm(x, block["ln_2.weight"], block["ln_2.bias"]), block, params, quantized_params)
    return x
...
def load_safetensors_state(model_dir):
    ...
    if bin_candidates:
        ...
        for candidate in bin_candidates:
            state.update(torch.load(candidate, map_location="cpu", weights_only=True))
        return state

    raise FileNotFoundError(
        f"Could not find a checkpoint in {model_dir}. "
        "Expected model.safetensors or compatible checkpoint shards."
    )
...
def load_or_quantize(name, array, params, quantized_params, quantized_names, group_size):
    if should_quantize(array, group_size):
        quantized_params[name] = quantize_affine_int4(array, group_size=group_size)
        quantized_names.append(name)
    else:
        params[name] = Tensor(array, requires_grad=False)


def init_model_from_hf(model_name="gpt2", rank=4, alpha=8.0, quant_group_size=64):
    ...
    load_or_quantize(
        "wte.weight",
        np.asarray(state[wte_key], dtype=np.float32),
        params,
        quantized_params,
        quantized_names,
        quant_group_size,
    ...
    load_or_quantize(
        "wpe.weight",
        np.asarray(state[wpe_key], dtype=np.float32),
        params,
        quantized_params,
        quantized_names,
        quant_group_size,
    )
    params["ln_f.weight"] = Tensor(np.asarray(state[ln_f_weight_key], dtype=np.float32), requires_grad=False)
    params["ln_f.bias"] = Tensor(np.asarray(state[ln_f_bias_key], dtype=np.float32), requires_grad=False)
    ...
    for layer in range(n_layer):
        ...
        for name in hf_weight_key_names(layer):
            ...
            load_or_quantize(
                name,
                np.asarray(state[hf_name], dtype=np.float32),
                params,
                quantized_params,
                quantized_names,
                quant_group_size,
    ...
    return params, quantized_params, lora_params, quantized_names, config
...
def block_view(params, layer):
    ...
    return {
        ...
        "attn.c_attn.weight_name": prefix + "attn.c_attn.weight",
        ...
        "attn.c_proj.weight_name": prefix + "attn.c_proj.weight",
        ...
        "mlp.c_fc.weight_name": prefix + "mlp.c_fc.weight",
        ...
        "mlp.c_proj.weight_name": prefix + "mlp.c_proj.weight",
...
def gpt_forward(input_ids, attn_mask, params, quantized_params, n_head, n_layer):
    _, seq_len = input_ids.shape
    # GPT-2 token and position embeddings.
    tok = get_weight(params, quantized_params, "wte.weight")[input_ids]
    pos = get_weight(params, quantized_params, "wpe.weight")[np.arange(seq_len)]
    x = tok + pos.reshape(1, seq_len, -1)

    for layer in range(n_layer):
        x = transformer_block(x, attn_mask, block_view(params, layer), n_head, params, quantized_params)

    x = layer_norm(x, params["ln_f.weight"], params["ln_f.bias"])
    # GPT-2 language-model head reuses the token embedding matrix.
    return x @ get_weight(params, quantized_params, "wte.weight").transpose(1, 0)
...
def sample_text(prompt, tokenizer, params, quantized_params, n_head, n_layer, block_size, max_new_tokens=80, temperature=0.8, top_k=40, stream=False):
    ...
    for _ in range(max_new_tokens):
        ...
        logits = gpt_forward(input_ids, attn_mask, params, quantized_params, n_head, n_layer).data
...
def count_float_params(params):
    return sum(value.data.size for value in params.values() if isinstance(value, Tensor))


def count_quantized_params(quantized_params):
    return sum(entry["num_values"] for entry in quantized_params.values())


def quantized_storage_bytes(quantized_params):
    return sum(
        entry["packed"].nbytes + entry["scales"].nbytes + entry["biases"].nbytes
        for entry in quantized_params.values()
    )


def main():
    parser = argparse.ArgumentParser(description="Load GPT-2 weights and train LoRA adapters in NumPy with packed 4-bit base weights.")
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt2",
        help="Hugging Face repo ID or local directory containing config.json and checkpoint weights",
    )
    ...
    parser.add_argument("--save-lora", type=str, default="numpy_gpt2_lora_quant4.npz")
    ...
    params, quantized_params, lora_params, quantized_names, hf_config = init_model_from_hf(
        args.model,
        rank=args.rank,
        alpha=args.alpha,
    ...
    total_params = count_float_params(params) + count_quantized_params(quantized_params)
    ...
    print(f"quantized tensors: {len(quantized_names)} at 4-bit")
    print(f"trainable params: {trainable_params} / {total_params}")
    print(f"packed quantized storage: {quantized_storage_bytes(quantized_params) / (1024 * 1024):.2f} MiB")
    ...
    if args.mode == "train":
        ...
        for step in range(1, args.steps + 1):
            ...
            optimizer.zero_grad()
            logits = gpt_forward(input_ids, attn_mask, params, quantized_params, hf_config["n_head"], hf_config["n_layer"])
