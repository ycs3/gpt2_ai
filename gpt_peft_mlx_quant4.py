...
def ensure_quantization_support():
    if not hasattr(mx, "quantize") or not hasattr(mx, "dequantize"):
        raise RuntimeError(
            "This MLX build does not expose mx.quantize/mx.dequantize. "
            "Upgrade MLX to a version with quantization support."
        )
...
def quantize_weight(weight, bits, group_size):
...
def should_quantize(weight, group_size):
    return len(weight.shape) >= 2 and weight.shape[-1] % group_size == 0


def dequantize_weight(quantized_entry):
    return mx.dequantize(
        quantized_entry["qweight"],
        quantized_entry["scales"],
        quantized_entry["biases"],
        group_size=quantized_entry["group_size"],
        bits=quantized_entry["bits"],
        mode="affine",
        dtype=mx.float32,
    )
...
def get_weight(params, quantized_params, name):
    if name in quantized_params:
        return dequantize_weight(quantized_params[name])
    return params[name]
...
def load_or_quantize(name, weight, params, quantized_params, quantized_names, bits, group_size):
    if should_quantize(weight, group_size):
        quantized_params[name] = quantize_weight(weight, bits=bits, group_size=group_size)
        quantized_names.append(name)
    else:
        params[name] = weight


def init_model_from_hf(model_name="gpt2", rank=4, alpha=8.0, quant_bits=4, quant_group_size=64):
    ensure_quantization_support()
    ...
    load_or_quantize(
        "wte.weight",
        mx.array(state[wte_key].astype(np.float32)),
        params,
        quantized_params,
        quantized_names,
        quant_bits,
    ...
    load_or_quantize(
        "wpe.weight",
        mx.array(state[wpe_key].astype(np.float32)),
        params,
        quantized_params,
        quantized_names,
        quant_bits,
    ...
    for layer in range(n_layer):
        ...
        for name in hf_weight_key_names(layer):
            ...
            tensor = mx.array(state[hf_name].astype(np.float32))
            load_or_quantize(
                name,
                tensor,
                params,
...
def gpt_forward(input_ids, attn_mask, params, quantized_params, n_head, n_layer):
    ...
    pos = get_weight(params, quantized_params, "wpe.weight")[mx.arange(seq_len)]
    ...
    lm_head = get_weight(params, quantized_params, "wte.weight").T
    return x @ lm_head
...
def sample_text(prompt, tokenizer, params, quantized_params, n_head, n_layer, block_size, max_new_tokens=80, temperature=0.8, top_k=40, stream=False):
    ...
    for _ in range(max_new_tokens):
        ...
        logits = gpt_forward(input_ids, attn_mask, params, quantized_params, n_head, n_layer)
...
def loss_fn(trainable_params, input_ids, labels, attn_mask, params, quantized_params, n_head, n_layer):
    merged_params = merge_params(params, trainable_params)
    logits = gpt_forward(input_ids, attn_mask, merged_params, quantized_params, n_head, n_layer)
    return cross_entropy(logits, labels)


def count_array_elements(tree):
    total = 0
...
def count_quantized_elements(quantized_params):
    return sum(
        int(np.prod(entry["scales"].shape)) * entry["group_size"]
        for entry in quantized_params.values()
    )


def main():
    parser = argparse.ArgumentParser(description="Load GPT-2 safetensors weights, quantize the base model to 4-bit, and train LoRA adapters in MLX.")
    ...
    parser.add_argument("--save-lora", type=str, default="mlx_gpt2_lora_quant4.npz")
    ...
    parser.add_argument("--quant-bits", type=int, default=4, choices=[4])
    parser.add_argument("--quant-group-size", type=int, default=64)
    args = parser.parse_args()

    np.random.seed(args.seed)

    params, quantized_params, lora_param_names, quantized_names, hf_config = init_model_from_hf(
        args.model,
        rank=args.rank,
        alpha=args.alpha,
    ...
    total_params = count_array_elements(params) + count_quantized_elements(quantized_params)
    ...
    print(f"quantized tensors: {len(quantized_names)} at {args.quant_bits}-bit")
