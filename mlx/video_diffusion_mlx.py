...
COND_DROP_PROB = 0.1
GUIDANCE_SCALE = 3.0
WEIGHTS_PATH = "video_diffusion_ucf101_mlx.safetensors"
...
class SmallVideoUNet(nn.Module):
    def __init__(
        self,
...
def prompt_to_labels(prompts, class_names):
    ...
    for prompt in prompts:
        ...
        if match is None:
            ...
            raise ValueError(f"Could not map prompt '{prompt}' to a class. Try one of: {valid}")
        label_ids.append(match)
    return mx.array(np.array(label_ids, dtype=np.int32))


def loss_fn(model, clips, labels, schedule):
    ...
    predicted_noise = model(noisy_clips, timesteps, labels=labels, cond_mask=keep_mask)
    return nn.losses.mse_loss(predicted_noise, true_noise)


def show_video_grid(samples, labels=None, class_names=None, title="Generated video clips", save_path=None, show=True):
    clips = np.clip((np.array(samples) * 0.5) + 0.5, 0.0, 1.0)
    count = len(clips)
    num_frames = clips.shape[1]
    fig, axes = plt.subplots(count, num_frames, figsize=(num_frames * 1.5, count * 1.8))
    ...
    for row in range(count):
        for col in range(num_frames):
            ...
            if col == 0 and labels is not None and class_names is not None:
                label_idx = int(np.array(labels)[row])
                axes[row, col].set_title(class_names[label_idx], fontsize=8)
...
def show_forward_process(clips, schedule, save_path="forward_video_diffusion_mlx.png", show=True):
    ...
    fig.suptitle("Forward diffusion (video -> noise)", fontsize=13)
    plt.tight_layout()
    if save_path:
        plt.savefig(save_path, dpi=150)
        print(f"saved {save_path}")
...
def load_training_data(args):
    ...
    if test_paths:
        ...
        test_clips, test_labels, _ = load_video_dataset(
            test_paths, test_labels, args.num_frames, args.image_size
        )
    return train_clips, train_labels, test_clips, test_labels, class_names, train_paths
...
def train(args):
    ...
    train_clips, train_labels, test_clips, test_labels, class_names, _ = load_training_data(args)
    schedule = make_schedule()
    model = SmallVideoUNet(
        in_channels=CHANNELS,
    ...
    steps_per_epoch = max(len(train_clips) // args.batch_size, 1)
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_clips, batch_labels in batch_iterator(
            ...
            loss, grads = loss_and_grad_fn(model, batch_clips, batch_labels, schedule)
        ...
        if epoch % args.sample_every == 0 or epoch == 1:
            ...
            show_video_grid(
                ...
                class_names=class_names,
                title=f"Epoch {epoch}",
                save_path=f"samples_video_epoch{epoch:02d}.png",
                show=not args.no_show,
            )

    model.save_weights(args.weights_path)
    print(f"saved model weights to {args.weights_path}")
    return model, schedule, class_names, train_clips


def build_parser():
    parser = argparse.ArgumentParser(
        description="Train or sample a small UCF101 class-conditional video diffusion model in MLX."
    )
...
def main():
    ...
    if args.mode == "train":
        model, schedule, class_names, train_clips = train(args)
        demo_labels = None if args.unconditional else prompt_to_labels(args.prompts, class_names)
        demo_samples = sample(
            model,
            schedule,
            class_names=class_names,
            labels=demo_labels,
            num_samples=args.num_samples if args.unconditional else None,
            guidance_scale=args.guidance_scale,
            sample_steps=args.sample_steps,
            ddim_eta=args.ddim_eta,
            image_size=args.image_size,
        ...
        show_video_grid(
            ...
            labels=demo_labels,
            class_names=class_names,
            title="Prompted video samples" if demo_labels is not None else "Unconditional video samples",
            save_path="samples_video_prompted_mlx.png" if demo_labels is not None else "samples_video_unconditional_mlx.png",
            show=not args.no_show,
        )
        ...
        show_video_grid(
            final_samples,
            title="Final generated video clips",
            save_path="final_video_diffusion_mlx.png",
            show=not args.no_show,
        )
    ...
    train_paths, _, _, _, class_names = load_dataset_metadata(args)
    schedule = make_schedule()
    model = SmallVideoUNet(
        in_channels=CHANNELS,
        base_channels=args.base_channels,
    ...
    if args.unconditional:
        ...
        show_video_grid(
            samples,
            title="Unconditional video samples",
            save_path="samples_video_unconditional_mlx.png",
            show=not args.no_show,
        )
    else:
        ...
        show_video_grid(
            ...
            labels=prompt_labels,
            class_names=class_names,
            title="Prompted video samples",
            save_path="samples_video_prompted_mlx.png",
            show=not args.no_show,
        )
